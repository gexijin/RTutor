...
read_additional_data <- function() {
  ...
  # only keep the first 40 characters
  events$Event <<- substr(events$Event, 1, 30)
  ...
  medications$Medication <<- as.factor(medications$Medication)
...
prep_input <- function(txt, selected_data, df, use_python) {
  ...
  if (!is.null(selected_data)) {
    if (selected_data != no_data) {
      ...
      relevant_df <- names(relevant_df[relevant_df])

      
      # select data "demography"
      txt <- paste(txt, 
        "Use the df data frame.",
        describe_df(eval(parse(text=selected_data)))
      )

      relevant_df <- relevant_df[relevant_df != selected_data]

      if( "demography" %in% relevant_df) {
        txt <- paste(txt, 
          "\nThe demography data frame contains information about patient demographics information.",
          describe_df(demography)
        )
      }

      if( "events" %in% relevant_df) {
        txt <- paste(txt, 
